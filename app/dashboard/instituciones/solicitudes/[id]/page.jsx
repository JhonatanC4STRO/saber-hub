...
import DetalleSolicitudInstitucion from '@/components/admin/DetalleSolicitudInstitucion';

export default async function DetalleSolicitudPage({ params }) {
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center p-6 text-center" style={{ fontFamily: 'Inter, sans-serif' }}>
        ...
        <p className="text-[#6B7280] mt-3 text-[14px] leading-relaxed max-w-md">
          No tienes permisos para ver el detalle de esta solicitud de institución. Esta sección está reservada exclusivamente para los administradores globales de la plataforma.
        </p>
        <Link 
          href="/dashboard" 
    ...
    <div className="min-h-screen bg-[#FFFFFF] font-sans">
      ...
      <main className="max-w-[1440px] mx-auto px-6 md:px-8 pt-10 pb-20">
        <div className="bg-white border border-[#F3F4F6] border-b-2 border-b-[#1E40AF] rounded-[4px] p-8 shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
          <DetalleSolicitudInstitucion solicitud={JSON.parse(JSON.stringify(solicitud))} />
        </div>
      </main>
      <FooterAdmin />
    </div>
